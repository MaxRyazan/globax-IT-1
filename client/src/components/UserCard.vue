...
const openPopup = () => {
    ...
    card.value.classList.add('selected')
}
const closePopup = () => {
    isPopupOpen.value = false
    card.value.classList.remove('selected')
}

</script>
<template>
    <div @click="openPopup" ref="card" class="card" title="Нажмите, чтобы увидеть полную информацию!">
        <h2 class="card__title">{{ user.name }}</h2>
        <div class="card__item item" style="padding-left: 4px">
            ...
            <input style="padding-left: 5px" class="item__input" readonly type="tel" :value="user.phone">
        </div>
        <div class="card__item item">
            <svg class="item__mail" width="24" height="14" viewBox="0 0 24 14" fill="none"
            ...
            <input class="item__input" readonly type="email" :value="user.email">
        </div>
    </div>
    <the-popup :user v-if="isPopupOpen" @close="closePopup"/>
    <div class="blocker" v-if="isPopupOpen" @click="closePopup"></div>
</template>
